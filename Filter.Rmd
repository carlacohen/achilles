...
TO DO
Finish parameterising figures type
Finish figure dimensions
...
ini <- read_yaml("filter_pipeline.yml")
...
```{r}
#set default to not print code and alter figure dimensions according to the number of files
knitr::opts_chunk$set(echo = FALSE, fig.width=10, fig.height=length(file_list)) 
...
#save the plot
ggsave(paste0("Filtered_Figures.dir/EmptyDrops_libsize.", ini$file_type), p, device = ini$file_type, width = 10, height = length(file_list), bg = "white")

```

#### What is the distribution of empty droplets by library size and number of features?

```{r, fig.height=length(file_list)*1.5, fig.width=14, message=FALSE, warning = FALSE}
plot_list <- list()
...
#save the plot
ggsave(paste0("Filtered_Figures.dir/EmptyDrops_distribution.", ini$file_type), p, device = ini$file_type, width = 10, height = length(file_list), bg = "white")
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/EmptyDrops_cellnumber.", ini$file_type), p, device = ini$file_type, 
       width = 10, height = length(file_list)*1.2, bg = "white")
...
For the achilles, MSK1284-Ach-MB and MSK1284-Ach-MTJ have very low cell numbers (<200). 
Also MSK1284-Ach-Enth and MSK1250-Ach_MB2 failed QC at the multiqc stage so should be removed. 
...
files_to_remove <- ini$files_to_remove
...
```{r, fig.width=12, fig.height=length(file_list)*2, message = FALSE}
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Ambient_RNA.", ini$file_type), p, device = ini$file_type, width = 12, height = length(file_list)*2, bg = "white")
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Doublets_libsize.", ini$file_type), p, device = ini$file_type, width = 10, height = length(file_list)*1.5, bg = "white")
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Doublets_Ambient_cellnumber.", ini$file_type), p, device = ini$file_type, 
       width = 10, height = length(file_list)*1.2, bg = "white")
...
```{r, fig.width=length(file_list)+(15/length(file_list)), fig.height=14, warning=FALSE}
...
p1 <- ggplot(df_all, aes(x = orig.ident, y = nCount_RNA, fill = low_lib_size))+
    geom_violin(trim = FALSE, width = 1, position = position_dodge(0))+
    scale_y_log10() + 
    scale_fill_viridis_d(begin = 0, end = 0.75)+
    theme_cowplot()+
    theme(axis.text.x = element_text(angle = 45, hjust=1))+
...
#add an overall title and arrange the graphs on one page
title <- ggdraw() + draw_label("Statistical filtering after empty droplet removal", fontface='bold', size = 16)
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Statistical_filtering.", ini$file_type), p, device = ini$file_type, 
       width = length(file_list)+(15/length(file_list)), height = 12, bg = "white")
...
```{r, fig.height=length(file_list)*1.5, fig.width=14, message = FALSE}
...
#save the plot
ggsave(paste0("Filtered_Figures.dir/Statistical_filtering.", ini$file_type), p, device = ini$file_type, 
       width = 16, height = length(file_list)*2, bg = "white")
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Statistical_outliers_ambient_doublet.", ini$file_type), p, device = ini$file_type, 
       width = length(file_list)+(15/length(file_list)), height = 14, bg = "white")
...
#add an overall title and arrange the graphs on one page
title <- ggdraw() + draw_label("Statistical outliers after ambient RNA & doublet removal", fontface='bold', size = 16)
p <- plot_grid(plotlist = plot_list, label_size = 24, ncol = 3)
p <- plot_grid(title, p, ncol=1, rel_heights=c(0.05, 1))
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Statistical_filtering_ambient_doublets_removed.", ini$file_type), p, device = ini$file_type, 
       width = 16, height = length(file_list)*2, bg = "white")
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Discard_cellnumber.", ini$file_type), p, device = ini$file_type, width = 10, height = length(file_list), bg = "white")
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Novelty_filtering.", ini$file_type), p, device = ini$file_type, 
       width = 12, height = length(file_list), bg = "white")
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Highly_variable_genes.", ini$file_type), p, device = ini$file_type, 
       width = 12, height = length(file_list)*2, bg = "white")
```
...
```{r, warning=FALSE, message = FALSE, fig.width = 12, fig.height=length(file_list)*2}
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Outliers_UMAP.", ini$file_type), p, device = ini$file_type, 
       width = 12, height = length(file_list)*2, bg = "white")
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/Highly_variable_genes_filtered.", ini$file_type), p, device = ini$file_type, 
       width = 10, height = length(file_list), bg = "white")
```
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/UMAP_after_filtering.", ini$file_type), p, device = ini$file_type, 
       width = 12, height = length(file_list), bg = "white")
```



### Plot the nCountRNA, nFeature_RNA and percent_mt on the UMAP
...
for(i in 1:length(sce_decontX_doublet_discard_filtered)){
    ...
    plot_list[[i]] <- plotReducedDim(sce_decontX_doublet_discard_filtered[[i]], dimred = "UMAP", colour_by = "percent_mt")+
        scale_colour_gradientn(colours = viridis(256, option = "D"), name = "percent_mt")+
        ggtitle(project_title[[i]])
    
}

#add an overall title and arrange the graphs on one page
title <- ggdraw() + draw_label("Percent mitochondrial reads after filtering", fontface='bold', size = 16)
p <- plot_grid(plotlist = plot_list, label_size = 16, ncol = 3)
p <- plot_grid(title, p, ncol=1, rel_heights=c(0.05, 1))
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/UMAP_percent_mt.", ini$file_type), p, device = ini$file_type, 
       width = 12, height = length(file_list), bg = "white")
```
...
for(i in 1:length(sce_decontX_doublet_discard_filtered)){
    ...
    plot_list[[i]] <- plotReducedDim(sce_decontX_doublet_discard_filtered[[i]], dimred = "UMAP", colour_by = "nCount_RNA")+
        scale_colour_gradientn(colours = viridis(256, option = "D"), name = "percent_mt")+
        ggtitle(project_title[[i]])
...
#add an overall title and arrange the graphs on one page
title <- ggdraw() + draw_label("nCount_RNA after filtering", fontface='bold', size = 16)
p <- plot_grid(plotlist = plot_list, label_size = 16, ncol = 3)
p <- plot_grid(title, p, ncol=1, rel_heights=c(0.05, 1))
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/UMAP_nCount_RNA.", ini$file_type), p, device = ini$file_type, 
       width = 12, height = length(file_list), bg = "white")
```
...
for(i in 1:length(sce_decontX_doublet_discard_filtered)){
    ...
    plot_list[[i]] <- plotReducedDim(sce_decontX_doublet_discard_filtered[[i]], dimred = "UMAP", colour_by = "nFeature_RNA")+
        scale_colour_gradientn(colours = viridis(256, option = "D"), name = "percent_mt")+
        ggtitle(project_title[[i]])
...
#add an overall title and arrange the graphs on one page
title <- ggdraw() + draw_label("nFeature_RNA after filtering", fontface='bold', size = 16)
p <- plot_grid(plotlist = plot_list, label_size = 16, ncol = 3)
p <- plot_grid(title, p, ncol=1, rel_heights=c(0.05, 1))
...
```{r, include=FALSE}
#save the plot
ggsave(paste0("Filtered_Figures.dir/UMAP_nFeature_RNA.", ini$file_type), p, device = ini$file_type, 
       width = 12, height = length(file_list), bg = "white")
```


### Save the RDS objects & Additional filtering
...
Following clustering we may wish to look at the object with all percent_mt filtered to a certain level e.g. 5%
...
decontX_doublet_discard_filtered_SeuratObject.rds: as sce_decontX_doublet_discard_filtered
decontX_doublet_low_filtered_SeuratObject.rds: as sce_decontX_doublet_low_filtered
...
for(i in 1:length(sce_empty)){
    ...
    # filter to percent_mt threshold and save
    so_mt_filter[[i]] <- subset(so[[i]], subset = percent_mt < ini$percent_mt_filter)
    
    saveRDS(so_mt_filter[[i]], 
            paste0("RDS_objects.dir/filtered/", project_title[[i]], "_decontX_doublet_discard_", ini$percent_mt_filter, "pctmt_SeuratObject.rds"))
